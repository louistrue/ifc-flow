...
import { X, Plus, Save, FileDown, TagIcon, HelpCircle } from "lucide-react"
import { workflowStorage } from "@/lib/workflow-storage"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Alert, AlertDescription } from "@/components/ui/alert"

export function SaveWorkflowDialog({ open, onOpenChange, onSave, onSaveLocally, flowData, existingWorkflow }) {
  ...
  const [localFilename, setLocalFilename] = useState(existingWorkflow?.name || "untitled-workflow")
  ...
  const handleOpenChange = (open: boolean) => {
    if (open) {
      ...
      setLocalFilename(existingWorkflow?.name?.toLowerCase().replace(/\s+/g, "-") || "untitled-workflow")
  ...
  const handleSaveLocally = () => {
    ...
    const filename = localFilename.trim().endsWith(".json") ? localFilename.trim() : `${localFilename.trim()}.json`
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          ...
          <DialogDescription>Save your workflow to the library or as a local file</DialogDescription>
        </DialogHeader>

        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "library" | "local")} className="mt-2">
          ...
          <TabsContent value="library" className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="max-w-xs">Tags help you organize and find workflows later</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
            ...
            <Alert className="bg-primary/5 border-primary/20">
              <AlertDescription className="text-xs flex items-center gap-1.5">
                ...
                Workflows saved to the library are stored in your browser's local storage
              </AlertDescription>
            </Alert>
          </TabsContent>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  ...
                  autoFocus
                />
                <span className="text-muted-foreground whitespace-nowrap">.json</span>
              </div>
            </div>
            <Alert className="bg-primary/5 border-primary/20">
              <AlertDescription className="text-xs">
                ...
                <div className="pl-5">
                  You can later import this file back into Grasshopper for IFC using the workflow library.
                </div>
              </AlertDescription>
            </Alert>
          </TabsContent>
        ...
        <DialogFooter className="mt-4">
          ...
          {activeTab === "library" ? (
            ...
            <Button onClick={handleSaveLocally} disabled={!localFilename.trim()}>
              Download File
            </Button>
          )}
        </DialogFooter>
