...
import { getActiveViewer } from "@/lib/ifc/viewer-registry"; // Import registry getter
    ...
    try {
      switch (node.type) {
        case "ifcNode":
          ...
          if (node.data?.model?.elements) {
            console.log(`ifcNode ${node.id}: Using model data already present.`);
            result = node.data.model;
          } else {
            console.log(`ifcNode ${node.id}: Model data not found, loading via filename.`);
            const filename = node.data?.properties?.filename;
            if (!filename) { result = { type: "error", value: "No filename/model" }; break; }
            const fileObject = getIfcFile(filename);
            if (!fileObject) { result = { type: "error", value: `File not found: ${filename}` }; break; }
            console.log(`ifcNode ${node.id}: Loading file ${filename}...`);
            try {
              result = await loadIfcFile(fileObject);
              console.log(`ifcNode ${node.id}: Async load complete for ${filename}.`);
            } catch (err: any) {
              console.error(`ifcNode ${node.id}: Async load failed for ${filename}:`, err);
              result = { type: "error", value: `Load failed: ${err.message}` };
            }
          }
          break;
        ...
        case "analysisNode":
          ...
          const primaryInputAnalysis = inputValues.input;
          const referenceInputAnalysis = inputValues.reference;
          ...
          if (!primaryElementsAnalysis || !Array.isArray(primaryElementsAnalysis)) {
            result = { type: "error", value: "Invalid primary elements input for analysis" }; break;
          }
          if (!referenceElementsAnalysis || !Array.isArray(referenceElementsAnalysis)) {
            result = { type: "error", value: "Invalid reference elements input for analysis" }; break;
          }

          // Directly call performClashDetection - it will handle the readiness check internally
          ...
          result = await performClashDetection(
            primaryElementsAnalysis,
            referenceElementsAnalysis,
            { tolerance, showIn3DViewer: true }
          );
          // Log the immediate result, which might be 'viewer_not_ready'
          console.log(`AnalysisNode ${node.id} immediate result:`, result?.status || 'Completed');
          // No need to log clashes found here if status isn't completed
          if (result?.status !== 'viewer_not_ready') {
      ...
      result = { type: "error", value: `Execution Error: ${error instanceof Error ? error.message : String(error)}` };
    }

    // Assign result (unless it was set asynchronously, like in the old ifcNode loading)
    if (result !== undefined) {
      this.nodeResults.set(nodeId, result);
    }
    // processNode should return the *immediate* result (or promise if applicable)
    // The handling of async results within ifcNode needs review, but let's return `result` for now
    return result;
  }
